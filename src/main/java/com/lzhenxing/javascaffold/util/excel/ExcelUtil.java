...
public class ExcelUtil {
     ...
     * @since JDK 1.6
     */
    private <T> List<T> buildJaveBeans(Workbook workbook, Class<?> clazz) throws ExcelParsingException {
...
//        }
        ...
        for (int currentLocation = excelMappingInfo.getDataStart(); excelMappingInfo.getDataEnd() == 0
            ...
            for (Field mappedField : mappedExcelFields) {
                ...
                List<?> fieldValue = buildJaveBeans(workbook, fieldType.equals(List.class) ? getFieldType(mappedField)
                        : fieldType);
    ...
    private <T> T getNewInstance(Sheet sheet, Class<?> clazz, ExcelMappingInfo excelMappingInfo,
    ...
    private <T> void buildExcel(List<?> list, Class<T> clazz, ExcelMappingInfo excelMappingInfo, Sheet sheet,
                                CellStyle tableHeadStyle, CellStyle dataStyle) throws IllegalArgumentException, IllegalAccessException,
    ...
    private <T> T getInstance(Class<?> clazz) throws ExcelParsingException {
        T object = null;
        try {
        ...
        for (Field field : fields) {
            ...
            if (excelFieldMappingByHeadName != null) {
                if (excelMappingInfo.getHeadNamePosition(excelFieldMappingByHeadName.headName()) != null) {
                    ...
                    excelFieldMappingInfo.setPosition(excelMappingInfo.getHeadNamePosition(excelFieldMappingByHeadName
                            .headName()));
